...
final class HyperIDSDKAuthRS256Test: HyperIDSDKAuthTestBase {
	...
	override func FactoryHyperIDAPIAuth() async throws -> HyperIDAPIAuth {
		...
		guard let items = items, error == noErr, CFArrayGetCount(items) > 0 else { throw HyperIDAPIAuthError.invalidClientInfo() }
		...
		return try await HyperIDAPIAuth(clientInfo:		ClientInfo(clientId: 			"android-sdk-test-rsa",
