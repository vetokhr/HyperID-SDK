...
//**************************************************************************************************
//	StorageByEmailDataGet
//--------------------------------------------------------------------------------------------------
internal class StorageByEmailDataGet(private val owner		: HyperIDSDKStorageByEmailImpl,
									 val key				: String,
									 val completeListener	: IHyperIDSDKStorageByEmail.IDataGetResultListener) : RestApiRequest()
{
	override fun start()
	{
		owner.dataGet(this)
	...
	override fun requestSuccess(answer : Any)
	{
		...
		{
			if(answer.result == 0 || answer.result == 1)
			{
				if(answer.values.isNotEmpty())
				{
					completeListener.onRequestComplete(IHyperIdSDK.RequestResult.SUCCESS,
													   null,
													   IHyperIDSDKStorageByEmail.DataGetResult(UserDataGetResult.SUCCESS,
				...
				else
				{
					...
					completeListener.onRequestComplete(IHyperIdSDK.RequestResult.SUCCESS,
													   null,
													   IHyperIDSDKStorageByEmail.DataGetResult(UserDataGetResult.FAIL_BY_KEY_NOT_FOUND,
																							   key,
																							   null))
				}
			}
			else if(answer.result == -6)	//UserDataGetResult.FAIL_BY_KEY_ACCESS_DENIED)
			{
				completeListener.onRequestComplete(IHyperIdSDK.RequestResult.SUCCESS,
												   null,
												   IHyperIDSDKStorageByEmail.DataGetResult(UserDataGetResult.FAIL_BY_KEY_ACCESS_DENIED,
																						   key,
																						   null))
			}
			...
			{
				//FAIL_BY_INVALID_PARAMETERS | FAIL_BY_SERVICE_TEMPORARY_NOT_VALID | FAIL_BY_UNSUPPORTED_ERROR
