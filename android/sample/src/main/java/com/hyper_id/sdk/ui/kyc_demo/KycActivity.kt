...
class KycActivity : ComponentActivity()
{
	...
	override fun onCreate(savedInstanceState : Bundle?)
	{
		...
		val mfaViewModel : ViewModelKycDemo by viewModels { ViewModelKycDemo.Factory }
			...
			Hyper_sdkTheme()
			{
				...
				})
				{
					Surface(modifier = Modifier
							...
							.padding(it))
					{
						UiSceneKycApiDemo(_viewModel = mfaViewModel)
					}
				}
			}
		}
...
@Composable
fun UiSceneKycApiDemo(_viewModel : ViewModelKycDemo)
{
	Column(modifier = Modifier.fillMaxWidth())
	{
		BoxedButton(text	= "Get status BASIC",
					onClick	= { _viewModel.kycApi.userStatusGet(KycVerificationLevel.BASIC) })
		BoxedButton(text	= "Get status FULL",
					onClick	= { _viewModel.kycApi.userStatusGet(KycVerificationLevel.FULL) })
		BoxedButton(text	= "Get topLevel status",
					onClick	= { _viewModel.kycApi.userStatusTopLevelGet() })
	}
}

@Preview(name = "")
@Composable
fun PreviewUiSceneKycApiDemo()
{
	UiSceneKycApiDemo(ViewModelKycDemo(HyperIdSdkKycDemoApi(object : IHyperIDSDKKYC
			{
